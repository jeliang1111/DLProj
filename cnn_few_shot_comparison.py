...
def main():
    ...
    # --- feature scaler: fit on the 1 HNEI training battery ---
    # We fit on the single training battery to avoid any leakage from the test set.
    feat_scaler = StandardScaler()
    feat_scaler.fit(hnei_train_battery[0][FEATURE_COLS].values)

    def scale_segs(segs: list[pd.DataFrame]) -> list[pd.DataFrame]:
        return scale_segments(segs, feat_scaler)

    hnei_train_scaled = scale_segs(hnei_train_battery)
    hnei_test_scaled  = scale_segs(hnei_test_segs)
    nasa_scaled       = scale_segs(nasa_segments)
